<?php include "config/dbconnection.php"; 
    ...
    if(isset($_GET['client_id'])){
        ...
        // if($query_up){
            $status = "Approved";
            $reset_query = "UPDATE bank_withdraw SET status ='$status' WHERE b_id='$bank_id'";
            $reset_result = mysqli_query($connected,$reset_query);
            if($reset_result){
                $upSuc = "User bank withdrawal was approved successfully";
                header("Location: withdrawal-request?upSuc=".$upSuc);
                exit();
            }else{
                $upErr = "Unable to approve user bank withdrawal contact webmaster";
